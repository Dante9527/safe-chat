...
原本的 lazy init 把所有初始化（PyTorch import、`intfloat/multilingual-e5-base` 模型載入、ChromaDB HNSW 索引讀取）全堆在第一次請求上，加起來 10-30 秒。移到 lifespan 後，`/api/health` 通過時所有組件已就緒，用戶首次請求不再有冷啟動。
...
通用型 reranker 對中文法規專業術語的判斷不如 embedding 模型（`intfloat/multilingual-e5-base` 是針對中文最佳化的）。Reranker 把原本排對的結果重新排錯：
...
**在中文法規 RAG 場景下，通用 reranker 弊大於利。** Embedding 模型（multilingual-e5-base）對法規術語的語意排序已經足夠好，cross-encoder reranker 反而引入雜訊。
...
## 6. 為什麼用 Vue 3 CDN 而不是 Vite + npm build？

### 決策：單一 HTML 檔部署

`templates/index.html` 包含所有 CSS、Vue 程式碼、Vue CDN 引用。**不需要 node_modules、不需要 build step、不需要 webpack 設定**。

### 理由

1. **Demo 簡化** — 展示不用解釋 build pipeline
